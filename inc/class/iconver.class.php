...
本文件用于编码转换，使用示例：
$chs=new iconver();
$str=$chs->Convert($str, 'GBK', 'BIG5');
$str=$chs->Convert($str, 'BIG5', 'UTF8');
echo $str;
*/
...
class iconver{
	...
	var $TableEncode	=	array();
	var $IndexPoint		=	array(
		'GBKtoUTF8'		=>	0,
		'GBKtoUNICODE'	=>	0,
		'UTF8toGBK'		=>	512,
		'UTF8toBIG5'	=>	1536,
	...
	function initConvert($SourceLang, $TargetLang){
		if(($SourceLang=$this->_getCharset($SourceLang)) && ($TargetLang=$this->_getCharset($TargetLang)) && $SourceLang!=$TargetLang){
			...
			$this->IconvEnabled=(function_exists('iconv') && !in_array($TargetLang, array('BIG5')))?1:0;
			!$this->IconvEnabled && $this->TableHandle=fopen($this->_getCharsetFilePath().'encode.table', 'r');
		}
	}
	
	function _getCharset($lang){
	...
	function _CHStoUTF8($srcText){
		$this->_getTableIndex();$tarText='';
		...
		fclose($this->TableHandle);
		return $tarText;
	}
	
	function _UTF8toCHS($srcText){
		$this->_getTableIndex();
		$tarText='';$i=0 ;
	...
	function _CHStoUNICODE($srcText, $SourceLang=''){
		$tarText='';
	...
	function Convert($srcText, $SourceLang, $TargetLang){
		$this->initConvert($SourceLang, $TargetLang);
		
		switch($this->EncodeLang){
			case 'GBKtoUTF8':
				return $this->IconvEnabled?iconv('GBK', 'UTF-8//IGNORE', $srcText):$this->_CHStoUTF8($srcText);
				break;
			case 'BIG5toUTF8':
				return $this->IconvEnabled?iconv('BIG5', 'UTF-8//IGNORE', $srcText):$this->_CHStoUTF8($srcText);
				break;
			case 'UTF8toGBK':
				return $this->IconvEnabled?iconv('UTF-8', 'GBK//IGNORE', $srcText):$this->_UTF8toCHS($srcText);
				break;
			case 'UTF8toBIG5':
				return $this->_UTF8toCHS($srcText);
				break;
			case 'GBKtoUNICODE':
				return $this->_CHStoUNICODE($srcText, 'GBK');
				break;
			case 'BIG5toUNICODE':
				return $this->_CHStoUNICODE($srcText, 'BIG5');
				break;
			case 'GBKtoBIG5':
			case 'BIG5toGBK':
	...
	function CHSConvert($srcText, $SourceLang='GBK'){
		if(strtoupper(substr($SourceLang, 0, 3))=='GBK'){
			$handle=fopen($this->_getCharsetFilePath().'gb_big5.table', 'r');
		}else{
			$handle=fopen($this->_getCharsetFilePath().'big5_gb.table', 'r');
		}
		$encode=array();
	...
	function _getCharsetFilePath(){
		global $c;
		return $c['root_path'].'/inc/file/iconver/';
	}
}
?>