...
class dianxiaomi_api{
	...
	*************************************************************************************************************/
	public static function sync_get_orders(){
		...
		$page_count=(int)$p_Count?(int)$p_Count:100;
		($page_count>1000 || $page_count<=0) && $page_count=100;
		$start_row=(int)($page-1)*$page_count;
		...
		(int)$p_UpdateStartTime && $where.=" and UpdateTime>'$p_UpdateStartTime'";
		(int)$p_UpdateEndTime && $where.=" and UpdateTime<'$p_UpdateEndTime'";
		$row_count=db::get_row_count('orders', $where, 'OrderId');
		$orders_row=db::get_limit('orders', $where, '*', 'OrderId desc', $start_row, $page_count);
		...
		foreach($orders_row as $v){
			...
			$v['ManageCurrency'] && $v['Currency']=$v['ManageCurrency'];
	...
	*************************************************************************************************************/
	public static function sync_orders_track(){
		...
		foreach((array)$orders_row as $k=>$v){
			$order_ary[$v['OId']]=$v;
			orders::orders_log($v['UserId'], '店小秘API', $v['OrderId'], 5, 'Update order status from Awaiting Shipping to Shipment Shipped', 1);
		}
		//更新数据
		$result_data=array();
		foreach((array)$Trackinfo as $k=>$v){
			...
			if($order_ary[$k]){//有订单数据
				...
				$order_row=db::get_one('orders', "OId='{$OId}'", 'OrderStatus');
				if(!$order_row){//订单不存在
					$result_data[$k]=0;
				}elseif(@in_array($order_row['OrderStatus'], array(5,6))){//订单已发货
					$result_data[$k]=-1;
				}elseif($order_row['OrderStatus']<4){//订单未付款
					$result_data[$k]=-2;
				}elseif($order_row['OrderStatus']==7){//订单已取消
					$result_data[$k]=-3;
				}
			}
		}
	...
	/*************************************************************************************************************
	主控制面板请求【修改运单号】【主控制面板->网站】
	*************************************************************************************************************/
	public static function sync_mod_orders_track(){
		global $c;
		@extract($_POST, EXTR_PREFIX_ALL, 'p');
		//初始化
		!$p_Trackinfo && ly200::e_json('没有运单号数据', 0);
		...
		//订单信息
		$order_ary=array();
		$orders_row=db::get_all('orders', $where." and OrderStatus=5");
		foreach((array)$orders_row as $k=>$v){
			$order_ary[$v['OId']]=$v;
		}
		//更新数据
		...
		foreach((array)$Trackinfo as $k=>$v){
			...
			if($order_ary[$k]){//有订单数据
				$OrderId=$order_ary[$OId]['OrderId'];
				db::update('orders', "OrderId='{$OrderId}'", array('TrackingNumber'=>addslashes(stripslashes($v)),'UpdateTime'=>$c['time']));
				$result_data[$OId]=1;
			}else{//没订单数据
				$result_data[$k]=0;
			}
		...
		//记录日志
		foreach((array)$orders_row as $k=>$v){
			orders::orders_log($v['UserId'], '店小秘API', $v['OrderId'], 5, 'Update Delivery Number', 1);
		}
		//返回结果
		ly200::e_json($result_data, 1);
	}
	...
	*************************************************************************************************************/
	public static function sync_get_products_category(){
		...
		$row_count=db::get_row_count('products_category', 1, 'CateId');
		$total_page=ceil($row_count/$page_count);
	...
	*************************************************************************************************************/
	public static function sync_get_products_list(){
		...
		$page=(int)$p_Page?(int)$p_Page:1;
		$page_count=(int)$p_Count?(int)$p_Count:100;
		($page_count>100 || $page_count<=0) && $page_count=100;
		$start_row=(int)($page-1)*$page_count;
		//初始化
		...
		$row_count=db::get_row_count('products', $where, 'ProId');
		$total_page=ceil($row_count/$page_count);
		$products_row=db::get_limit('products', $where, $Column, 'ProId asc', $start_row, $page_count);
