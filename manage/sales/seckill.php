...
<div class="r_nav">
	...
	?>
		...
		<ul class="ico">
			<?php if($permit_ary['add']){?><li><a class="tip_ico_down add" href="./?m=sales&a=seckill&d=add" label="{/global.add/}"></a></li><?php }?>
...
<div id="sales" class="r_con_wrap">
	...
	?>
		...
		<table border="0" cellpadding="5" cellspacing="0" class="r_con_table">
			<thead>
				<tr>
					...
					<td width="5%" nowrap="nowrap">{/sales.package.remainder_qty/}</td>
			...
			<tbody>
				...
				?>
				<tr>
					<?php if($permit_ary['del']){?><td nowrap="nowrap"><input type="checkbox" name="select" value="<?=$v['SId'];?>" class="va_m" /></td><?php }?>
					<td class="left">
						<div class="p_row">
					...
					<td nowrap="nowrap"><?=$v['Qty'];?></td>
					<td nowrap="nowrap"><?=$v['RemainderQty']==0?'<span class="fc_red">'.$v['RemainderQty'].'</span>':$v['RemainderQty'];?></td>
					...
					<?php if($permit_ary['edit'] || $permit_ary['del']){?>
						<td nowrap="nowrap">
							<?php if($permit_ary['edit']){?><a class="tip_ico tip_min_ico" href="./?m=sales&a=seckill&d=edit&SId=<?=$v['SId'];?>" label="{/global.edit/}"><img src="/static/ico/edit.png" align="absmiddle" /></a><?php }?>
							<?php if($permit_ary['del']){?><a class="tip_ico tip_min_ico del" href="./?do_action=sales.seckill_del&SId=<?=$v['SId'];?>" label="{/global.del/}" rel="del"><img src="/static/ico/del.png" align="absmiddle" /></a><?php }?>
						</td>
					<?php }?>
				</tr>
				<?php }?>
		...
		<div id="turn_page"><?=manage::turn_page($seckill_row[1], $seckill_row[2], $seckill_row[3], '?'.ly200::query_string('page').'&page=');?></div>
	...
	?>
		...
		<div class="list_box">
			<div class="lefter">
				<form id="tuan_form">
				<div class="p_title mar_t_0">{/sales.package.product_seckill_area/}</div>
				<div class="rows">
				...
				<div class="p_related_frame p_frame">
					<div class="p_related_notice">{/sales.package.seckill_notice/}</div>
				</div>
				<div class="related_bottom">
					<div class="related_btn">
						<input type="submit" class="btn_ok submit_btn fr" name="submit_button" value="{/global.submit/}" />
						<a href="./?m=sales&a=seckill" class="btn_cancel fr">{/global.return/}</a>
					</div>
					<div class="clear"></div>
				</div>
			...
			<div class="list_box_righter">
				...
				<div class="product_frame p_frame">
					...
					<div id="product_item_<?=$proid;?>" class="product_item" pro_num="<?=$proid;?>">
						<div img_num="<?=$proid;?>" id="p_img_<?=$proid;?>" class="p_img"><img src="<?=$img;?>" width="118" height="118" alt="<?=$name;?>" /></div>
						<div class="p_info">
    ...
    <?php
		...
		$id_list && $where.=" and SId in ({$id_list})";
						...
						<div id="related_product_<?=$ProId; ?>" class="p_related_item">
							...
							<div class="p_related_info">
								...
								<div class="related_list related_big_list">
									{/sales.package.seckill_price/}: <?=$c['manage']['currency_symbol']?><input name="Price[]" type="text" value="<?=$v['Price']; ?>" class="form_input" size="5" maxlength="5" notnull="">&nbsp;&nbsp;
									{/products.products.qty/}: <input name="Qty[]" type="text" value="<?=$v['Qty']; ?>" class="form_input" size="5" maxlength="5" notnull="">&nbsp;&nbsp;
									{/sales.package.max_qty/}: <input name="MaxQty[]" type="text" value="<?=$v['MaxQty']; ?>" class="form_input" size="5" maxlength="5" notnull=""></div>
								<div class="related_list related_big_list">{/sales.package.remainder_qty/}: <input name="RemainderQty[]" type="text" value="<?=$v['RemainderQty']; ?>" class="form_input" size="5" maxlength="5" notnull=""></div>
	...
	<div class="edit_bd list_box">
		<form id="seckill_edit_form" name="seckill_edit_form" class="r_con_form">
			<div class="rows_box">
				<div class="rows">
				...
				<div class="rows">
					...
					<span class="input"><input name="PromotionTime" value="<?=date('Y-m-d H:i:s',($seckill_row['StartTime']?$seckill_row['StartTime']:$c['time'])).'/'.date('Y-m-d H:i:s', ($seckill_row['EndTime']?$seckill_row['EndTime']:$c['time']));?>" type="text" class="form_input" size="36" readonly></span>
				</div>
				<div class="rows">
					<label>{/sales.package.seckill_price/}</label>
					<span class="input">
						<span class="price_input"><b><?=$c['manage']['currency_symbol']?><div class="arrow"><em></em><i></i></div></b><input name="Price" value="<?=$seckill_row['Price'];?>" type="text" class="form_input" size="10" maxlength="10" rel="amount" notnull /></span>
					</span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/products.products.qty/}</label>
					<span class="input"><input name="Qty" type="text" value="<?=$seckill_row['Qty'];?>" class="form_input" size="5" maxlength="5"></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/sales.package.remainder_qty/}</label>
					<span class="input"><input name="RemainderQty" type="text" value="<?=$seckill_row['RemainderQty'];?>" class="form_input" size="5" maxlength="5"></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/sales.package.max_qty/}</label>
					<span class="input"><input name="MaxQty" type="text" value="<?=$seckill_row['MaxQty'];?>" class="form_input" size="5" maxlength="5"></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label></label>
					<span class="input">
						...
						<a href="./?m=sales&a=seckill" class="btn_cancel">{/global.return/}</a>
					</span>
					<div class="clear"></div>
				</div>
			</div>
