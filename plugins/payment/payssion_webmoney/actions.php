 ...
 * 
 * @package     payment
 * @subpackage  payssion - webmoney (Payssion本地支付)
 * @category    payment
 * @author      鄙人
 ...
 */
class payssion_webmoney_actions 
{ 
    ...
    function __construct(&$pluginManager){
        ...
        //第三个是插件所执行的方法 
        $pluginManager->register('payssion_webmoney', $this, '__config'); 
        $pluginManager->register('payssion_webmoney', $this, 'do_payment'); 
        $pluginManager->register('payssion_webmoney', $this, 'notify'); 
        $pluginManager->register('payssion_webmoney', $this, 'returnUrl'); 
    }
    ...
    function do_payment($data){
		...
		$pm_currency_ary=array('EUR', 'RUB', 'GBP', 'CHF', 'NOK', 'DKK', 'SEK', 'JPY', 'USD');//所支付的货币（欧洲）
		...
		$form_data=array( 
			...
			'pm_id'			=>	'webmoney',	//支付方式id: qiwi, yamoney, sofort, paysafecard, trustpay, molpay, boleto_br
			...
			'sub_track_id'	=>	'',	//额外跟踪信息
			'description'	=>	"{$data['domain']}{$data['order_row']['OId']}",	//订单描述, 最长256字符, 通过将网址和货物名称组合起来
			'redirect_url'	=>	"{$data['domain']}/payment/payssion_webmoney/returnUrl/?utm_nooverride=1",	//支付未完成、失败等同步通知
			'success_url'	=>	"{$data['domain']}/payment/payssion_webmoney/returnUrl/?utm_nooverride=1",	//支付成功同步通知
			'notify_url'	=>	"{$data['domain']}/payment/payssion_webmoney/notify/"	//异步通知
		);
    ...
    function notify($data){
		...
		$account=str::json_data(db::get_value('payment', "Method='PayssionWebmoney'", 'Attribute'), 'decode');
		...
		$log=ob_get_contents();
		ob_end_clean();
		file::write_file('/_pay_log_/payssion/Webmoney/'.date('Y_m/', $c['time']), "{$OId}-".rand(1,1000).".txt", $log);	//把返回数据写入文件
    }
