 ...
 * 
 * @package     payment
 * @subpackage  payssion - bradesco (Payssion本地支付)
 * @category    payment
 * @author      鄙人
 ...
 */
class payssion_bradesco_actions 
{ 
    ...
    function __construct(&$pluginManager){
        ...
        //第三个是插件所执行的方法 
        $pluginManager->register('payssion_bradesco', $this, '__config'); 
        $pluginManager->register('payssion_bradesco', $this, 'do_payment'); 
        $pluginManager->register('payssion_bradesco', $this, 'notify'); 
        $pluginManager->register('payssion_bradesco', $this, 'returnUrl'); 
    }
    ...
    function do_payment($data){
		...
		$form_data=array( 
			...
			'pm_id'			=>	'bradesco_br',	//支付方式id: qiwi, yamoney, sofort, paysafecard, trustpay, molpay, boleto_br
			...
			'redirect_url'	=>	"{$data['domain']}/payment/payssion_bradesco/returnUrl/?utm_nooverride=1",	//支付未完成、失败等同步通知
			'success_url'	=>	"{$data['domain']}/payment/payssion_bradesco/returnUrl/?utm_nooverride=1",	//支付成功同步通知
			'notify_url'	=>	"{$data['domain']}/payment/payssion_bradesco/notify/"	//异步通知
		);
    ...
    function notify($data){
		...
		$account=str::json_data(db::get_value('payment', "Method='PayssionBradesco'", 'Attribute'), 'decode');
		...
		$log=ob_get_contents();
		ob_end_clean();
		file::write_file('/_pay_log_/payssion/Bradesco/'.date('Y_m/', $c['time']), "{$OId}-".rand(1,1000).".txt", $log);	//把返回数据写入文件
    }
