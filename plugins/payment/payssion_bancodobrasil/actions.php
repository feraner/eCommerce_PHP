 ...
 * 
 * @package     payment
 * @subpackage  payssion - bancodobrasil (Payssion本地支付)
 * @category    payment
 * @author      鄙人
 ...
 */
class payssion_bancodobrasil_actions 
{ 
    ...
    function __construct(&$pluginManager){
        ...
        //第三个是插件所执行的方法 
        $pluginManager->register('payssion_bancodobrasil', $this, '__config'); 
        $pluginManager->register('payssion_bancodobrasil', $this, 'do_payment'); 
        $pluginManager->register('payssion_bancodobrasil', $this, 'notify'); 
        $pluginManager->register('payssion_bancodobrasil', $this, 'returnUrl'); 
    }
    ...
    function do_payment($data){
		...
		$pm_currency_ary=array('BRL', 'USD');//所支付的货币（巴西）
		...
		$form_data=array( 
			...
			'pm_id'			=>	'bancodobrasil_br',	//支付方式id: qiwi, yamoney, sofort, paysafecard, trustpay, molpay, boleto_br
			...
			'redirect_url'	=>	"{$data['domain']}/payment/payssion_bancodobrasil/returnUrl/?utm_nooverride=1",	//支付未完成、失败等同步通知
			'success_url'	=>	"{$data['domain']}/payment/payssion_bancodobrasil/returnUrl/?utm_nooverride=1",	//支付成功同步通知
			'notify_url'	=>	"{$data['domain']}/payment/payssion_bancodobrasil/notify/"	//异步通知
		);
    ...
    function notify($data){
		...
		$account=str::json_data(db::get_value('payment', "Method='PayssionBancodobrasil'", 'Attribute'), 'decode');
		...
		$log=ob_get_contents();
		ob_end_clean();
		file::write_file('/_pay_log_/payssion/Bancodobrasil/'.date('Y_m/', $c['time']), "{$OId}-".rand(1,1000).".txt", $log);	//把返回数据写入文件
    }
