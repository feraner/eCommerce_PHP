 ...
 * 
 * @package     payment
 * @subpackage  payssion - santander (Payssion本地支付)
 * @category    payment
 * @author      鄙人
 ...
 */
class payssion_santander_actions 
{ 
    ...
    function __construct(&$pluginManager){
        ...
        //第三个是插件所执行的方法 
        $pluginManager->register('payssion_santander', $this, '__config'); 
        $pluginManager->register('payssion_santander', $this, 'do_payment'); 
        $pluginManager->register('payssion_santander', $this, 'notify'); 
        $pluginManager->register('payssion_santander', $this, 'returnUrl'); 
    }
    ...
    function do_payment($data){
		...
		$form_data=array( 
			...
			'pm_id'			=>	'santander_br',	//支付方式id: qiwi, yamoney, sofort, paysafecard, trustpay, molpay, boleto_br
			...
			'redirect_url'	=>	"{$data['domain']}/payment/payssion_santander/returnUrl/?utm_nooverride=1",	//支付未完成、失败等同步通知
			'success_url'	=>	"{$data['domain']}/payment/payssion_santander/returnUrl/?utm_nooverride=1",	//支付成功同步通知
			'notify_url'	=>	"{$data['domain']}/payment/payssion_santander/notify/"	//异步通知
		);
    ...
    function notify($data){
		...
		$account=str::json_data(db::get_value('payment', "Method='PayssionSantander'", 'Attribute'), 'decode');
		...
		$log=ob_get_contents();
		ob_end_clean();
		file::write_file('/_pay_log_/payssion/Santander/'.date('Y_m/', $c['time']), "{$OId}-".rand(1,1000).".txt", $log);	//把返回数据写入文件
    }
