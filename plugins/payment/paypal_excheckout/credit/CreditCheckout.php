<?php
//初始化
$OId=$p_web_info['OId'];//订单号
$CUSTOM=$p_web_info['CUSTOM'];//自定义字段，用“|”分割，【产品数量、国家ID、快递ID、快递名、快递类型、是否购买快递保险、快递运费、快递保险费、优惠券代码】
...
$shipping_ary=array(
	...
	'ShippingPrice'			=>	(float)$p_transactions[0]['amount']['details']['shipping'],//快递运费
	'ShippingInsurancePrice'=>	(float)$p_transactions[0]['amount']['details']['insurance'],//快递保险费
);
//记录数据
if(strtolower($p_state)=='created'){
	//交易已成功创建
	file::write_file('/_pay_log_/paypal_excheckout/credit_log/'.date('Y_m/d/', $c['time']), "{$OId}-GetExpress.txt", date('Y-m-d H:i:s', $c['time'])."\r\n\r\n".(ly200::is_mobile_client(1)==1?'Mobile':'PC')."\r\n\r\n".print_r($_POST, true));
}else{
	//交易请求失败
	file::write_file('/_pay_log_/paypal_excheckout/credit_log/'.date('Y_m/d/', $c['time']), "{$OId}-error.txt", date('Y-m-d H:i:s', $c['time'])."\r\n\r\n".(ly200::is_mobile_client(1)==1?'Mobile':'PC')."\r\n\r\n".print_r($_POST, true));
	ly200::e_json('The transaction request failed.', 0);
}
//订单数据
...
$CouponCutPrice	= (float)$p_transactions[0]['amount']['details']['shipping_discount'];//折扣金额
$Currency		= $p_transactions[0]['amount']['currency'];//币种
$CountryAcronym	= $p_payer['payer_info']['country_code'];//国家缩写编码
//转换后台默认汇率
...
$shipping_ary['ShippingPrice']=cart::currency_price($shipping_ary['ShippingPrice'], $ExchangeRate, $_SESSION['ManageCurrency']['ExchangeRate']);
$shipping_ary['ShippingInsurancePrice']=cart::currency_price($shipping_ary['ShippingInsurancePrice'], $ExchangeRate, $_SESSION['ManageCurrency']['ExchangeRate']);
$CouponCutPrice=cart::currency_price($CouponCutPrice, $ExchangeRate, $_SESSION['ManageCurrency']['ExchangeRate']);
...
if($Acronym!=strtoupper($CountryAcronym)){
	...
	$log=ob_get_contents();
	ob_end_clean();
	file::write_file('/_pay_log_/paypal_excheckout/credit_log/'.date('Y_m/d/', $c['time']), "{$OId}-error.txt", $log);	//把返回数据写入文件

	ly200::e_json('Parameters(Country) in session do not match those in PayPal API calls. (The Country of choice are inconsistent)', 0);
}
//设置Paypal买家账号的收货地址信息
$shipto_ary=array(
	'Account'		=>	$p_payer['payer_info']['email'],
	'FirstName'		=>	$p_payer['payer_info']['shipping_address']['recipient_name'],
	'LastName'		=>	'',
	'AddressLine1'	=>	$p_payer['payer_info']['shipping_address']['line1'],
	'AddressLine2'	=>	$p_payer['payer_info']['shipping_address']['line2'],
	'CountryCode'	=>	$country_row['Code'],
	'PhoneNumber'	=>	$p_payer['payer_info']['phone'],
	'City'			=>	$p_payer['payer_info']['shipping_address']['city'],
	'State'			=>	$p_payer['payer_info']['shipping_address']['state'],
	'Country'		=>	$country_row['Country'],
	'ZipCode'		=>	$p_payer['payer_info']['shipping_address']['postal_code'],
);
//会员信息
$user_data=user::check_login('', 1);
!$user_data && $user_data=array('UserId'=>0, 'Email'=>$p_payer['payer_info']['email']);
//地址信息
$address_ary=$bill_ary=array();
if((int)$user_data['UserId']){
	//设置收货地址
	$ship_row=str::str_code(db::get_one('user_address_book a left join country c on a.CId=c.CId left join country_states s on a.SId=s.SId', 'a.'.$user_data['fetch_where']." and a.IsBillingAddress=0 and a.CId='{$shipping_ary['ShippingCId']}'", 'a.*,c.Country,s.States as StateName'));
...
if((int)$user_data['UserId']){//实时查询当前会员等级
...
if((int)$c['config']['global']['LowConsumption'] && $_total_price<(float)cart::iconv_price($c['config']['global']['LowPrice'], 2)){
	...
	ly200::e_json($c['lang_pack']['cart']['error']['low_error'], 0);
}
//生成订单
$order_data=array(
	/*******************订单基本信息*******************/
...
//下单后自动注册会员
if((int)$c['config']['global']['AutoRegister'] && !db::get_row_count('user', "Email='{$user_data['Email']}'")){
	//随机生成密码
	$PasswordStr='';
