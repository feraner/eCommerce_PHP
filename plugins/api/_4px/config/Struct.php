...
class Struct{
	...
	public static function outputStruct($objectArr){//处理输出数据
		...
		foreach($objectArr as $key =>$value){
			if(gettype($value) == 'object'){
				...
				if(in_array($key, array('createDate', 'createPerson', 'occurAddress', 'occurDate', 'trackContent'))){
	...
	/******************************************* 在线订单操作 *******************************************/
	//创建订单
	public static function createOrderService($AUTHTOKEN, $customerParameter){//用php基类生成一个结构体【定义参数规则】
		//参数模
	...
	//预报订单
	public static function preAlertOrderService($AUTHTOKEN, $customerParameter){//用php基类生成一个结构体【定义参数规则】
		$temp = array(
				'arg0' => $AUTHTOKEN,
	...
	//创建并预报订单
	public static function createAndPreAlertOrderService($AUTHTOKEN, $customerParameter){//用php基类生成一个结构体【定义参数规则】
		//参数模
		$declareInvoice = array(
				"declareNote" => '',
				"declarePieces" => '',
	...
	//修改订单
	public static function modifyOrderService($AUTHTOKEN, $customerParameter){
		//参数模
		$declareInvoice = array(
				"declareNote" => '',
				"declarePieces" => '',
	...
	//删除订单
	public static function removeOrderService($AUTHTOKEN, $customerParameter){//用php基类生成一个结构体【定义参数规则】
		$temp = array(
				'arg0' => $AUTHTOKEN,
				'arg1' => $customerParameter//self::returnStructArr($customerParameter)
		);
	...
	//查询订单
	public static function findOrderService($AUTHTOKEN, $customerParameter){//这个方法没有结构体
		$findOrder = array(
				'orderNo' => '',
	...
	/*******在线订单操作工具********/
	//运费试算
	public static function chargeCalculateService($AUTHTOKEN, $customerParameter){//此方法没有结构体
	...
	//查询轨迹
	public static function cargoTrackingService($AUTHTOKEN, $customerParameter){//数组
		$temp = array(
				'arg0' => $AUTHTOKEN,
	...
	//申请拦截
	public static function cargoHoldService($AUTHTOKEN, $customerParameter){//数组
		$temp = array(
				'arg0' => $AUTHTOKEN,
				'arg1' => $customerParameter//这里直接用数组
		);
	...
	public static function findTrackingNumberService($AUTHTOKEN, $customerParameter){//数组
		$temp = array(
				'arg0' => $AUTHTOKEN,
				'arg1' => $customerParameter//这里直接用数组
		);
	...
	//打印标签 
	public static function printLableService($AUTHTOKEN, $customerParameter){//此接口学未实现
		$temp = array(
				'arg0' => $AUTHTOKEN,
				'arg1' => $customerParameter//这里直接用数组
		);
