...
if(! isset($token)) {
		...
		for($i=0;$i<count($L_NAME);$i++)
		{
			...
			$L_amt.='&L_PAYMENTREQUEST_0_AMT'.$i.'='.cart::iconv_price($L_AMT[$i], 2);
		...
		*/
		...
		$returnURL =urlencode("{$domain}/cart/excheckout/ReviewOrder.html");
		...
		*/
		...
		$nvpstr.="&PAYMENTREQUEST_0_AMT=".cart::iconv_price($amt, 2);//支付总金额
		$nvpstr.="&PAYMENTREQUEST_0_ITEMAMT=".cart::iconv_price($itemamt, 2);//订单所有物品的价格
