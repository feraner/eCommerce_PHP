...
foreach($cate_row as $v){
	if($v['CateId']){
		...
		if($v['UId']!='0,'){
			...
			$c_TopCateId && $_attr_id=(int)db::get_value('products_category', "CateId='$c_TopCateId'", 'AttrId');
		}
...
<div class="widget group_promotion">
	<div class="gp_title">
		<ul>
			...
			?>
			<li<?=$i?'':' class="current"';?>><span data="<?=(int)$v['Type']?'promotion':'purchase';?>" data-id="<?=$i;?>"><?=$name;?></span></li>
			<?php
					++$i;
				}
			}?>
	...
	<div class="widget gp_list">
		...
		?>
				<div class="widget promotion_body<?=(int)$type?' gp_list_promotion':' gp_list_purchase';?><?=$i?' hide':'';?>" data-id="<?=$i;?>">
					<div class="master">
						<s></s>
						<div class="prod_img"><a class="pic_box" href="javascript:;"><img src="<?=ly200::get_size_img($products_row['PicPath_0'], '240x240');?>" title="<?=$Name;?>" alt="<?=$Name;?>" /><span></span></a></div>
						...
						<?php }else{?>
							<div class="prod_price hide"><input type="checkbox" id="group_<?=$ProId;?>" onclick="return false;" oldprice="<?=cart::iconv_price($MOQ*$oldPrice, 2, '', 0);?>" curprice="<?=cart::iconv_price($MOQ*$ItemPrice, 2, '', 0);?>" proid="<?=$ProId;?>" checked /></div>
						<?php }?>
					</div>
					<div class="suits">
						<ul>
							...
							<li<?=$k2+1==$length?' class="last"':''?>>
								<div class="prod_img"><a class="pic_box" href="<?=$url;?>" target="_blank"><img src="<?=$img;?>" title="<?=$name;?>" alt="<?=$name;?>" /><span></span></a></div>
								<div class="prod_name"><a href="<?=$url;?>" target="_blank" title="<?=$name;?>"><?=$name;?></a></div>
								<?php if(!$type){?>
									<div class="prod_price clearfix">
										<input type="checkbox" id="group_<?=$row['ProId'];?>" oldprice="<?=$_moq*$oldprice;?>" curprice="<?=$_moq*$_price;?>" price="<?=$cprice;?>" attrprice="0" moq="<?=$_moq;?>" proid="<?=$row['ProId'];?>" sales="<?=$is_promotion?1:0;?>" salesPrice="<?=($is_promotion && !$row['PromotionType'])?$row['PromotionPrice']:'';?>" discount="<?=($is_promotion && $row['PromotionType'])?$row['PromotionDiscount']:'';?>" isSeckill="<?=$IsSeck;?>" />
										&nbsp;<em class="currency_data PriceColor"><?=$_SESSION['Currency']['Symbol'];?></em>
										<span class="price_data PriceColor" data="<?=$price;?>"><?=cart::currency_format($_price, 1, $_SESSION['Currency']['Currency']);?></span>
									</div>
								<?php }?>
								...
								<dl class="attribute" data-combination="<?=(int)$row['IsCombination'];?>">
									...
									?>
										<dd>
											<select name="id[<?=$v3;?>]" id="attr_<?=$v3;?>" attr="<?=$v3;?>">
												...
												foreach((array)$c_all_value_ary[$v3] as $k4=>$v4){
													...
													$value=$combinatin_ary[$row['ProId']]["|{$v4['VId']}|"][1];
										...
										<?php }?>
										<input type="hidden" id="attr_hide_<?=$v3;?>" class="attr_hide" value="" />
										<input type="hidden" id="ext_attr_<?=$v3;?>" class="ext_attr" value="<?=htmlspecialchars(str::json_data($ext_ary[$row['ProId']]));?>" />
									<?php }?>
								</dl>
							</li>
							<?php }?>
					...
					<div class="info">
						...
						<div class="prod_price clearfix">
							<strong class="group_curprice clearfix"><em class="currency_data PriceColor"><?=$_SESSION['Currency']['Symbol'];?></em><span class="price_data PriceColor" data="<?=$type?$v['CurPrice']:$MOQ*$ItemPrice;?>"><?=cart::iconv_price(($type?$v['CurPrice']:$MOQ*$ItemPrice), 2);?></span></strong>
