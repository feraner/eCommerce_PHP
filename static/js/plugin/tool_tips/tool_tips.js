...
$.fn.tool_tips=function(obj, options){
	...
	$this.each(function(){
		...
		function(){
			if($(sel).size() == 0){
				$('#main .righter').prepend('<div id="tool_tips' + tid + '" class="tool_tips ' + options.toolClass + '"><span class="close"></span><span class="arrow"></span><div></div></div>');
			...
			case 'vertical':
				...
				break;
			case 'horizontal':
				offset.left = $this.offset().left - $('#main .menu').width();
				offset.top = $this.offset().top - $('#header').height() - ($('.r_nav').length ? $('.r_nav').height() : 0);
				if(offset.left < $t.width() / 2){
					$(sel).css({
						left: offset.left + width + 8//15
					});
					$(arrowSel).css({
						left: -7,
				...
				if(offset.top + height + $(sel).height() + 47 - $t.scrollTop() <= $t.height() || offset.top - $(sel).height() - 52 < $t.scrollTop()){
					$(sel).css({
						top: offset.top + ($('.r_nav').length ? 25: -25)
					});
					$(arrowSel).css({
						top: 25
					});
