...
var products_list_obj={
	...
	resize:function(){
		$index=0;
		$('.prod_list .clear').remove();
		if($('#main').outerWidth()>1000){
			$('.prod_list .prod_box').removeClass('first').each(function(){
				$index=parseInt($(this).attr('num'));
				if($index%5==0) $(this).addClass('first');
				if(($index+1)%5==0) $(this).after('<div class="clear"></div>');
			});
		}else{
			$('.prod_list .prod_box').removeClass('first').each(function(){
				$index=parseInt($(this).attr('num'));
				if($index%4==0) $(this).addClass('first');
				if(($index+1)%4==0) $(this).after('<div class="clear"></div>');
			});
		}
	},
